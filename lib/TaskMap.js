...
})("TaskMap", function moduleClosure(global, WebModule /*, VERIFY, VERBOSE */) {
"use strict";
...
function TaskMap_run(taskName,   // @arg String
    ...
    var runner = new WebModule["Task"](taskName, taskMap["groups"].length, callback); // as Junction
    var data = {
        runner: runner,
        map:    taskMap["map"],
...
function _nextTaskGroup(data) {
    ...
    case 0:
        ...
        break;
    case 1:
        var singleTask = new WebModule["Task"](group[0], 1, function(error) {
        ...
        break;
    default:
        var parallelTask = new WebModule["Task"](group.join(), group.length, function(error) {
                                    //if (!data.runner["finished"] && buffer !== data.runner["buffer"]) {
                                    //  if (buffer !== data.runner["buffer"]) {
        ...
        for (var i = 0, iz = group.length; i < iz; ++i) {
            _callUserTask( new WebModule["Task"](group[i], 1, parallelTask) );
        }
    }
