...
//{@assert
    ...
    _if(!_type(callback, "Function"),                       "Task(,callback)");
...
function Task_message(message) { // @arg String: message.
                                 ...
                                 // @help: Task#message
//{@assert
    _if(!_type(message, "String"), "Task#message(message)");
//}@assert
...
//{@assert
    ...
    _if(!_type(callback,  "Function"),      "Task.run(,,callback)");
...
//{@assert
    if ( line.length > 1000 || !_validate(line, taskMap) ) {
        return callback( new TypeError("Task.run(taskRoute)") );
    }
//}@assert
...
function _nextGroup(param) {
    ...
    function _callUserTask(param, taskName, junc, singleTask) {
        ...
        if (taskName in param.taskMap) {
            try {
                param.taskMap[taskName](task, param.arg); // function userTask(task, arg) { task.pass(); }
                if (singleTask) {
                    _nextGroup(param); // recursive call
                }
            } catch (err) {
...
function _type(value, types, keys) {
    ...
    function judge(type) {
        ...
        case "object":    return (keys && !hasKeys(value, keys)) ? false
