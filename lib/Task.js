...
Task["run"]       = Task_run;       // Task.run(taskRoute:String,
                                    ...
                                    //          options:Object = {}):Task
Task["loop"]      = Task_loop;      // Task.loop(object:Object,
                                    //           tick:Function,
                                    //           callback:Function/Junction,
                                    //           options:Object = {}):Task
// --- implement -------------------------------------------
...
function Task_loop(object,    // @arg Object: for-in loop data. { a: 1, b: 2, c: 3 }
                   tick,      // @arg Function: tick(task, object.key, object):void
                   callback,  // @arg Function/Junction: finished callback(err:Error, buffer:Array)
                   options) { // @arg Object(= {}): { arg, name, buffer }
                              //       options.arg    - Any(= null): task argument.
                              //       options.name   - String(= "Task.run"): junction task name.
                              ...
                              // @help: Task.run
//{@assert
    _if(!Valid.type(object, "Object"),   "Task.run(object)");
    _if(!Valid.type(tick,   "Function"), "Task.run(,tick)");
//}@assert
    ...
    var taskMap = {
            "_": function(task, arg, groupIndex) {
                tick(task, keys[groupIndex], object);
            }
        };

    return Task["run"](taskRoute, taskMap, callback, options);
}
