...
function _update(that, method) { // @ret this:
    ...
    if (_.state === "") { // task in progress.
        ...
        if (_.state) { // task was finished. "pass" or "miss" or "exit"
            if (_.junction) {
                _.callback[_.state](); // call Junction#pass()
                                       //  or  Junction#miss()
                                       //  or  Junction#exit()
            } else {
                // callback(err:Error, buffer:Array, task:Task)
...
//{@assert
    _if(!_isNumber(count) || count < 0, "invalid Task#extend(count)");
//}@assert
...
//{@assert
    _if(!_isNumber(count) || count < 0, "invalid Task#missable(count)");
//}@assert
...
function Task_dump(filter) { // @arg String(= ""): task name filter.
                             // @ret Object: task info snap shot(dead copy).
                             // @help: Task.dump
                             // @desc: dump snapshot.
//{@assert
...
function Task_run(taskRoute, // @arg String: route setting. "a > b + c > d"
                  ...
                  callback,  // @arg Function/Junction: callback(err:Error, buffer:Array, task:Task)
...
//}@assert
    ...
    //                   parallel execution
    if (taskRoute) {
        groupArray = JSON.parse("[[" + taskRoute.replace(/\s+/g, "").               // "a>b+c>d"
                                                 replace(/\+/g, ",").               // "a>b,c>d"
                                                 replace(/>/g, "],[").              // "a],[b,c],[d"
                                                 replace(/(\w+)/g, '"$1"') + "]]"); // '"a"],["b","c"],["d"'
    } else {
        groupArray = JSON.parse('[["' + Object.keys(taskMap).join('"],["') + '"]]');
    }

//{@assert
    if ( !_validate(groupArray, taskMap) ) {
...
//}@assert
    ...
    var junction = new Task(groupArray.length, callback, { "name": name, "buffer": buffer });

    return _proceed(junction, groupArray, taskMap, args, 0);
}

function _proceed(junction, groupArray, taskMap, args, groupIndex) {
    if (!junction["isFinished"]()) {

        var group = groupArray[groupIndex]; // group: [task, ...]
        ...
        var groupJunction = new Task(group.length, function(err) {
            if (err) {
                ...
                junction["pass"]();
                _proceed(junction, groupArray, taskMap, args, groupIndex + 1); // recursive call
            }
        }, { "buffer": junction["buffer"]() });

        group.forEach(function(taskName) {
            var task = new Task(1, groupJunction, { "name": taskName }); // buffer: SharedBuffer

            if (taskName in taskMap) {
                try {
                    // function taskName(task, args) { task.pass(); } -> ok
                    // function taskName(task, args) { task.miss(); } -> error
                    taskMap[taskName](task, args);
                } catch (err) {
                    junction.message(err["message"]);
                    task["miss"]();
                }
