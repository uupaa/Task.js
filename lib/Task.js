...
})("Task", function moduleClosure(global, WebModule, VERIFY /*, VERBOSE */) {
"use strict";
...
//}@dev
    var junction = _isJunction(callback || null);
    ...
    this._buffer    = junction ? this._callback["buffer"] // shared buffer
                               : [];                      // new buffer
...
Task["prototype"] = Object.create(Task, {
    ...
    "error":        { "set": Task_setError                            },    // Task#error:Error|String|null
...
function _updateState(that, addPassCount, addMissCount, nextState) {
    ...
    if (that._state) { // task was finished
        if ( _isJunction(that._callback) ) {
            that._callback["error"] = that._error; // bubble up error to junction
            that._callback[that._state]();         // bubble up state to junction. Call one of the methods (pass, miss, exit)
        } else {
            switch (that._state) {
...
function _isJunction(object) {
    if (object) {
        return object instanceof WebModule["Task"] ||
               object instanceof Task ||
               object.constructor.name === "Task";
    }
    return false;
}

function Task_setError(error) { // @arg Error|String|null
//{@dev
    if (VERIFY) {
        $valid($type(error, "Error|String|null"), Task_setError, "error");
    }
//}@dev
