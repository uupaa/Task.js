...
//}@assert
    ...
    //                           ~~~~~~~~~~~-~~~~~~~~~~~~~~~~~~~~~~~~ <--- job
    if (taskRoute) {
        job = JSON.parse("[[" + taskRoute.replace(/\s+/g, "").               // "a>b+c>d"
                                          replace(/\+/g, ",").               // "a>b,c>d"
                                          replace(/>/g, "],[").              // "a],[b,c],[d"
                                          replace(/(\w+)/g, '"$1"') + "]]"); // '"a"],["b","c"],["d"'
    } else {
        job = JSON.parse('[["' + Object.keys(taskMap).join('"],["') + '"]]');
    }

//{@assert
    if ( !_validate(job, taskMap) ) {
...
//}@assert
    ...
    var junction = new Task(job.length, callback, { "name": name, "buffer": buffer });

    _recursiveProceed(junction, job, 0, taskMap, args);
    return junction;
}

function _recursiveProceed(junction, job, jobIndex, taskMap, args) {
    if (!junction["isFinished"]()) {

        // --- create task group ---
        var group = job[jobIndex]; // ["a"] or ["b", "c"] or ["d"]
        var groupJunction = new Task(group.length, function(err) {
                if (err) {
                    junction["message"](err["message"])["miss"]();
                } else {
                    junction["pass"]();
                    _recursiveProceed(junction, job, jobIndex + 1, taskMap, args); // recursive call
                }
        ...
        group.forEach(function(taskName) {
            ...
            if (taskName in taskMap) {
                try {
                 // taskName:function(task, args) { task.pass(); }
