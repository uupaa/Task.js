...
function Task_run(taskRoute, // @arg String: route setting. "a > b + c > d"
                  ...
                  callback,  // @arg Function/Junction: callback(err:Error, buffer:Array)
                  options) { // @arg Object(= {}): { args, name, buffer, inject }
                             //       options.args   - Object(= {}): task arguments.
                             //       options.name   - String(= "Task.run"): junction task name.
                             //       options.buffer - Array(= []): shared buffer.
                             //       options.inject - Integer(= 1000): Injecting some setTimeout to make break the Task too long.
                             // @ret Task: Junction
                             // @help: Task.run
//{@assert
    ...
    options && _if(!_isObject(options) ||
                   !_keys(options, { args: 1, name: 1, buffer: 1, inject: 1 }), "invalid Task.run(,,,options)");
//}@assert
    ...
    var inject = options["inject"] || 1000;

//{@assert
    _if(!_isObject(args),       "invalid Task.run(,,,options.args)");
    _if(!_isString(name),       "invalid Task.run(,,,options.name)");
    _if(!Array.isArray(buffer), "invalid Task.run(,,,options.buffer)");
    _if(!_isInteger(inject),    "invalid Task.run(,,,options.inject)");
//}@assert
    ...
    if (taskRoute) {
        ...
        line = JSON.parse('[["' + _injectContextSwitch(taskMap, inject).join('"],["') + '"]]');
    }
...
//}@assert
    ...
    var param = { junction: junction, line: line, index: 0, taskMap: taskMap, args: args };
...
function _nextGroup(param) {
    if (!param.junction["isFinished"]()) {
        ...
        if (group.length === 1) {
            ...
            var gjunc = new Task(group.length, function(err) {
                                ...
                                err || _nextGroup(param); // recursive call
    ...
    function _callUserTask(param, taskName, junc, singleTask) {
        ...
        if (taskName in param.taskMap) {
            try {
                param.taskMap[taskName](task, param.args); // function userTask(task, args) { task.pass(); }
                singleTask && _nextGroup(param); // recursive call
            } catch (err) {
                task["done"](err);
            }
...
function _injectContextSwitch(taskMap, inject) {
    var result = Object.keys(taskMap);
    var i = inject, iz = result.length;

    for (; i < iz; i += inject) {
        result.splice(i, 0, 0); // inject setTimeout(, 0)
    }
    return result;
}
