...
- 動的に taskCount を +1 するには、<span style="color:gold">task.extend(1)</span> とします
- 次々にユーザタスクが増えるケースなどで利用します

![](http://gyazo.com/b035eb1490bf7c56cb37a6db9f539e33.png)
...
![](http://gyazo.com/85fe1e3e873190deee7362c09c001d5a.png)

- 他のタスクを集約する Task を <span style="color:gold">Junction(合流点)</span> と呼びます
...
- このように Junction を使うと、複雑な待ち合わせ構造を簡単に記述できます。
- Junction はタスクの上下関係を記述する機能です
- 後述する Task.run はタスクの前後関係を記述する機能です
...
- **Junction** (上下関係の定義) と **Task.run** (前後関係の定義) を組み合わせると、色々な事が簡単になります
...
- シンプルかつ柔軟
- 様々な環境で利用できる
- 既存の構造を大きく改変しなくても導入できる
- 非同期処理の都合でコードを汚さずに済む

## 

(ε・◇・)з o O ( Task.js オススメ
